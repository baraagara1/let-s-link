...
session_start();
$id = $_SESSION['id'];// ID utilisateur avec valeur par défaut 2 pour les tests

$favorisController = new FavorisController();
$favoris = $favorisController->listerFavoris($id);

$eventController = new EventController();
$allEvents = $eventController->listEvents();

// Récupérer les détails complets des événements favoris
$favorisDetails = [];
foreach ($favoris as $fav) {
    foreach ($allEvents as $event) {
        if ($event['id_e'] == $fav['id_e']) {
            $favorisDetails[] = $event;
            break;
        }
    }
}
...
<a href="favoris.php?id=<?= $_SESSION['id'] ?>" class="nav-item nav-link">Mes Favoris</a>
    ...
    <div class="container-fluid page-header py-5 mb-5 wow fadeIn" data-wow-delay="0.1s">
        <div class="container text-center py-5">
            <h1 class="display-3 text-white mb-4 animated slideInDown">Mes Favoris</h1>
    ...
    <div class="container-xxl py-5">
        <div class="container">
            <div class="favorites-header wow fadeInUp" data-wow-delay="0.1s">
                <div class="heart-icon">
                    <i class="fas fa-heart"></i>
                </div>
                <h1 class="display-5 mb-3">Mes Événements Favoris</h1>
                <p class="fs-5">Retrouvez ici tous les événements que vous avez marqués comme favoris</p>
            </div>
            
            <div class="row">
            <?php if (empty($favorisDetails)): ?>
    <div class="col-12">
        <div class="no-favorites">
            <i class="far fa-heart fa-3x mb-3" style="color: #ff6b6b;"></i>
            <h3>Vous n'avez aucun favoris pour le moment</h3>
            <p>Explorez nos événements et ajoutez ceux qui vous intéressent à vos favoris.</p>
            <a href="Activités.php" class="btn btn-primary mt-3">Voir tous les événements</a>
        </div>
    </div>
<?php else: ?>
    <?php foreach ($favorisDetails as $event): ?>
                ...
                <div class="event-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" action="participer.php" style="display: inline;">
                            ...
                            <button type="submit" class="btn-action btn-participate">
                                <i class="fas fa-check me-1"></i> Participer
                            </button>
                        </form>
                        <form method="POST" action="supprimer_favoris.php" style="display: inline;">
                            <input type="hidden" name="id_e" value="<?= $event['id_e'] ?>">
                            <button type="submit" class="btn-action btn-remove" onclick="return confirm('Retirer cet événement de vos favoris ?')">
...
<?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Confirmation pour la suppression des favoris
